...
for i = 1:length(MtTracks) ;
   % c = c +i ; 
    ...
    timeM = idata(:, 25) ; % Extract Time in min
    ...
    l = length(pks) ;
    tID = idata(1:l, 19) ; % Get MtUniqueID that corresponds to these peaks
    ID = table2array(tID) ;
...
writetable(allpks, 'smoothpks.csv')
writetable(ndata, 'smoothdat.csv')
